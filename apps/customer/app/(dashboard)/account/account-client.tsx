...
export function AccountClient({
  ...
  const renderPayments = () => (
    <Card className="space-y-2 border border-hairline bg-white p-5 rounded-xl shadow-card">
      <h3 className="font-display text-base font-bold text-ink flex items-center gap-2">
  ...
  const renderCoupons = () => (
    <Card className="space-y-4 border border-hairline bg-white p-5 rounded-xl shadow-card">
      ...
      <ul className="space-y-3">
        {promos.map((promo) => (
          <li key={promo.code} className="border border-dashed border-accent/40 bg-accent/5 p-4 rounded-xl">
            <div className="flex items-center justify-between gap-3">
              ...
              <Badge tone={promo.status === 'available' ? 'accent' : 'muted'}>
                {promo.status}
              </Badge>
            </div>
            <p className="text-xs text-ink font-medium mt-1.5">
        ...
        <p className="text-xs text-muted">No promo codes are currently available or redeemed.</p>
      )}
    </Card>
  );
    ...
    <Card className="space-y-4 border border-hairline bg-white p-5 rounded-xl shadow-card text-center">
      ...
      <div className="space-y-2 mt-4 max-w-sm mx-auto">
        ...
        <p className="text-xs text-muted leading-relaxed">
          Give a friend your code - when their first booking completes, you get £5 wallet credit.
        </p>
      </div>
      {referralCode && (
      ...
      <div className="flex gap-2 justify-center max-w-sm mx-auto mt-4">
        <Button
          ...
          onClick={() => {
            const msg = `Give a friend your code - when their first booking completes, you get £5 wallet credit. My referral code is ${referralCode ?? ''}. https://urbanassist.co.uk`;
